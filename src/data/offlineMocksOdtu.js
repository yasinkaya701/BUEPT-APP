 ...
 *
 * Follows the official 2026 METU face-to-face format (verified against
 * epe.metu.edu.tr announcements and published breakdowns):
 *   - Listening (~15 min, short talks + dialogue + lecture excerpt)
 *   - Reading (60 min, dense academic texts — the dominant section)
 *   - Note-taking (lecture once → notes → answer afterwards)
 *   - Writing (ONE essay, ~35 min, idea guidelines provided)
 *   - Speaking (~8 min, face-to-face block — METU has it, BUSEPT does not)
 *
 * The shape reuses the same item types consumed by AIMockExamScreen
 * (short_answer, multiple_choice, matching, essay, speaking), so the
 * universal section extractor renders METU mocks without new UI code.
 */
...
const l1Reading = {
  ...
  questions: [
    ...
    shortAnswer('q4', 'How much did average test scores rise when the start time moved from 7:30 to 8:30, according to the graph note?', ['9 percent', 'nine percent', '9%']),
...
const l1NoteTaking = {
  ...
  questions: [
    shortAnswer('q1', 'What is the main advantage of solar energy mentioned in the lecture?', ['sunlight is free and unlimited', 'it works at night', 'no panels needed']),
    shortAnswer('q2', 'How many homes can one large wind turbine power?', ['about six hundred homes', '600 homes', 'six hundred']),
    ...
    shortAnswer('q5', 'Which energy source is described as stable but environmentally costly?', ['hydroelectric', 'hydro', 'water dams']),
  ],
};
...
const l1Speaking = {
  questions: [
    { id: 'sp1', type: 'speaking', q: 'Introduce yourself. Tell us about your hometown and your family.' },
    { id: 'sp2', type: 'speaking', q: 'Why did you choose this university and your department?' },
    { id: 'sp3', type: 'speaking', q: 'What are your hobbies, and how do they help you relax after studying?' },
    { id: 'sp4', type: 'speaking', q: 'Describe a memorable school experience from the past year.' },
  ],
};
...
const l2Listening = {
  ...
  questions: [
    shortAnswer('q1', 'What is the name of the phenomenon where new lanes quickly fill with traffic?', ['induced demand']),
    ...
    shortAnswer('q4', 'Roughly what share of trips in Copenhagen is made by bicycle?', ['a quarter', 'about 25 percent', '25%']),
...
const l2Reading = {
  ...
  questions: [
    ...
    shortAnswer('q4', 'By how many hours did average weekly screen time fall after switching to the ad-free service, according to the graph note?', ['6 hours', 'six hours']),
...
const l2NoteTaking = {
  ...
  questions: [
    ...
    shortAnswer('q2', 'How do vervets respond when they hear the eagle alarm call?', ['they look up and run into bushes', 'look up and hide in bushes']),
    ...
    shortAnswer('q4', 'Besides warning others, what possible side benefit do meerkat alarm calls serve, according to the lecture?', ['signal caller location to group members', 'signal their location']),
...
const l2Speaking = {
  questions: [
    { id: 'sp1', type: 'speaking', q: 'Tell us about yourself and the academic path that led you here.' },
    { id: 'sp2', type: 'speaking', q: 'What is the most important problem facing young people in your country today?' },
    { id: 'sp3', type: 'speaking', q: 'Some people prefer working in teams; others prefer working alone. What is your preference and why?' },
    { id: 'sp4', type: 'speaking', q: 'Describe a book, film, or project that changed the way you think about something.' },
  ],
};
...
const l3Listening = {
  ...
  questions: [
    shortAnswer('q1', 'Losses feel roughly how many times as painful as equivalent gains feel pleasant?', ['twice', 'two times', '2x']),
    shortAnswer('q2', 'What is the name of the effect where owners demand more to give up an object than they would pay to buy it?', ['endowment effect']),
    shortAnswer('q3', 'In the anchoring experiment, what did the rigged wheel influence participants to estimate?', ['the share of African nations in the UN', 'the share of African countries in the United Nations']),
...
const l3Reading = {
  ...
  questions: [
    ...
    shortAnswer('q4', 'What practical trade-off does the author identify about cognitive offloading?', ['frees capacity for higher-level thinking while outsourcing the foundations of thinking', 'frees capacity while outsourcing foundational knowledge']),
...
const l3NoteTaking = {
  ...
  questions: [
    shortAnswer('q1', 'What is the process called when slow-wave sleep transfers memories from the hippocampus to the neocortex?', ['systems consolidation']),
    ...
    shortAnswer('q4', 'Which sleep stage does alcohol before bed suppress disproportionately?', ['REM sleep', 'REM']),
...
const l3Speaking = {
  questions: [
    { id: 'sp1', type: 'speaking', q: 'Introduce yourself briefly and explain what you hope to achieve at university.' },
    { id: 'sp2', type: 'speaking', q: 'Do you think technology has made people more connected or more isolated? Support your view.' },
    { id: 'sp3', type: 'speaking', q: 'If you could change one thing about the education system in your country, what would it be and why?' },
    { id: 'sp4', type: 'speaking', q: 'Describe a time when you had to persuade someone. How did you approach it?' },
  ],
};
...
const l4Listening = {
  ...
  questions: [
    shortAnswer('q1', 'According to the lecturer, what is uncertainty in climate modelling?', ['a quantified feature of the models', 'a quantified feature']),
    shortAnswer('q2', 'Why are model results reported as scenario ensembles?', ['no model can know which emissions pathway society will choose', 'because society\'s emissions pathway is unknown']),
    shortAnswer('q3', 'What climate-system feature causes a ten-year plateau not to falsify a centennial trend?', ['internal variability']),
...
const l4Reading = {
  ...
  questions: [
    ...
    shortAnswer('q4', 'According to the text, what do Americans emphasise as the source of democratic failure?', ['state overreach against expression', 'government overreach on speech']),
...
const l4NoteTaking = {
  ...
  questions: [
    shortAnswer('q1', 'What is the name of the problem where journals preferentially publish significant findings?', ['publication bias', 'the file-drawer problem']),
    shortAnswer('q2', 'What practice involves trying multiple analyses and reporting the one that crosses the significance threshold?', ['p-hacking']),
    ...
    shortAnswer('q5', 'Name the three forces that produced the replication crisis, in the lecturer\'s order.', ['publication bias, p-hacking, low statistical power', 'publication bias, p-hacking, low power']),
  ],
};
...
const l4Speaking = {
  questions: [
    { id: 'sp1', type: 'speaking', q: 'Introduce yourself and summarise the academic contribution you most hope to make.' },
    { id: 'sp2', type: 'speaking', q: 'Some argue that higher education should be free; others say students should pay. What is your position?' },
    { id: 'sp3', type: 'speaking', q: 'How should societies balance freedom of expression with the need to protect vulnerable groups?' },
    { id: 'sp4', type: 'speaking', q: 'Describe a complex problem you solved. Walk us through your reasoning process.' },
  ],
};
