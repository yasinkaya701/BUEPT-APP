...
import { StyleSheet, Text, TextInput, View, useWindowDimensions, ScrollView, TouchableOpacity } from 'react-native';
...
import { checkOnlineFeedback, summarizeMatches, requestParaphrase, requestWritingRevision, requestSentenceBySentenceFeedback } from '../utils/onlineFeedback';
...
export default function FeedbackScreen({ navigation, route }) {
  ...
  // Launch deep AI tools immediately after initial report is ready
  useEffect(() => {
    if (autoGenerated && sourceText.trim()) {
      runWritingRevision();
      runSentenceAnalysis();
      runOnlineFeedback();
    }
  }, [autoGenerated, runWritingRevision, runSentenceAnalysis, runOnlineFeedback, sourceText]);
  ...
  const runSentenceAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const res = await requestSentenceBySentenceFeedback({
        text: sourceText,
        prompt: promptText
      });
      setSentenceAnalysis(res || []);
    } catch (err) {
      console.warn('Sentence analysis error:', err);
    } finally {
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const reply = await executeDirectAiChat({
        ...
        Essay: "${sourceText}"
        Prompt: "${promptText}"
        Report Summary: "${compactRubric.feedbackSummary}"
        
        Answer the student's question about their writing in detail, maintaining the context of the essay and the feedback already given.`,
        messages: [
          ...chatMessages.slice(-5), // Keep last 5 for context
          { role: 'user', content: userMsg }
        ]
      });
      setChatMessages(prev => [...prev, { role: 'assistant', content: reply }]);
    } catch (e) {
      setChatMessages(prev => [...prev, { role: 'assistant', content: `Chat error: ${e.message}` }]);
    } finally {
      ...
      <Card style={styles.card}>
        ...
        {revisionResult ? (
          <>
            ...
            {revisionResult.bueptBand || revisionResult.ieltsBand ? (
              <View style={styles.dualBandRow}>
                <View style={[styles.bandPill, { backgroundColor: colors.primary }]}>
                  <Text style={styles.bandPillLabel}>BUEPT: {revisionResult.bueptBand || '??'}</Text>
                  <Text style={styles.bandPillScore}>{revisionResult.bueptScore ?? '--'}/100</Text>
                </View>
                <View style={[styles.bandPill, { backgroundColor: colors.accent }]}>
                  <Text style={styles.bandPillLabel}>IELTS: {revisionResult.ieltsBand || '??'}</Text>
                  <Text style={styles.bandPillScore}>{revisionResult.ieltsScore ?? '--'}/9.0</Text>
                </View>
              </View>
            ) : null}

            {revisionResult.detailedMistakes?.length ? (
              <View style={styles.mistakeBlock}>
                <Text style={styles.bodyStrong}>Mistake-by-Mistake Analysis</Text>
                {revisionResult.detailedMistakes.map((m, idx) => (
                  <View key={`mistake-${idx}`} style={styles.mistakeItem}>
                    <View style={styles.mistakeHeader}>
                      <Text style={styles.mistakeRule}>{m.rule?.toUpperCase()}</Text>
                    </View>
                    <Text style={styles.mistakeOriginal}>Original: "{m.original}"</Text>
      ...
      {sentenceRevisionPlan.length ? (
        <Card style={styles.card}>
          ...
          {(sentenceRevisionPlan || []).map((item, index) => {
            if (!item) return null;
            return (
              <View key={item.id || index} style={styles.revisionItem}>
                <Text style={styles.sectionLabel}>Sentence {index + 1}</Text>
                <Text style={styles.bodyBlock}>{item.sentence || ''}</Text>
                <View style={styles.signalRow}>
                  {(item.focus || []).map((focus, fidx) => (
                    <View key={`${item.id || index}-${focus || fidx}`} style={[styles.signalPill, styles.signalPillGood]}>
                      <Text style={[styles.signalPillText, styles.signalPillTextGood]}>{focus || 'General'}</Text>
                    </View>
                  ))}
                </View>
                {(item.reasons || []).map((reason, ridx) => (
                  <Text key={`${item.id || index}-${reason || ridx}`} style={styles.body}>• {reason}</Text>
                ))}
  ...
  const renderLineByLine = () => (
    <>
      <Card style={styles.card}>
        <Text style={styles.h3}>Sentence-by-Sentence Audit</Text>
        <Text style={styles.sub}>Deconstruct your entire essay and see exactly how to upgrade every sentence to an academic level.</Text>
        {sentenceAnalysis.length === 0 && !sentenceLoading && (
          <Button label="Start Full Sentence Audit" onPress={runSentenceAnalysis} icon="analytics-outline" />
        )}
        {sentenceLoading && <Text style={styles.body}>AI is analyzing your sentence structures... please wait.</Text>}
        
        {sentenceAnalysis.map((item, idx) => {
          if (!item) return null;
          return (
            <View key={`line-${idx}`} style={styles.lineItem}>
              <View style={styles.lineNumber}><Text style={styles.lineNumberText}>{idx + 1}</Text></View>
              <View style={styles.lineContent}>
                <Text style={styles.lineOriginal}>"{item.original || ''}"</Text>
                <View style={styles.lineImprovedBox}>
                  <Ionicons name="sparkles" size={14} color={colors.success} />
                  <Text style={styles.lineImproved}>{item.improved || ''}</Text>
                </View>
                <Text style={styles.lineReason}>Why: {item.reason || 'N/A'}</Text>
              </View>
            </View>
          );
        })}
  ...
  return (
    <Screen scroll contentStyle={styles.content}>
      ...
      {activeTab === 'line-by-line' ? renderLineByLine() : null}
      ...
      <Card style={styles.chatCard}>
        <View style={styles.chatHeader}>
          ...
          <Text style={styles.h3}>Ask AI Coach</Text>
        </View>
        <Text style={styles.sub}>Have a question about a specific correction or score? Ask the AI directly.</Text>
        
        <ScrollView style={styles.chatList} showsVerticalScrollIndicator={false}>
          {chatMessages.map((msg, i) => {
            if (!msg) return null;
            return (
              <View key={i} style={[styles.chatBubble, msg.role === 'assistant' ? styles.chatBubbleAssistant : styles.chatBubbleUser]}>
                <Text style={[styles.chatText, msg.role === 'user' && { color: '#FFF' }]}>{msg.content || ''}</Text>
              </View>
            );
          })}
          {chatLoading && <Text style={styles.chatLoadingText}>AI is typing...</Text>}
        </ScrollView>

        <View style={styles.chatInputRow}>
          <TextInput
            style={styles.chatInput}
            placeholder="Ask why this was wrong..."
            value={chatInput}
