...
# The new_items from expand_speaking.py
new_items = [
    {
        "id": "sp_p1_05",
        "level": "P1",
...
for item in raw_data:
    if "prompts" in item and "topic" in item:
        # Explode BUEPT prompts
        for i, p_text in enumerate(item["prompts"]):
            # Normalize level (e.g. C1/P4 -> P4)
            level = item["level"].split('/')[-1] if '/' in item["level"] else item["level"]
            
            # Map ideal_clusters to vocab if they exist
            vocab = []
            if "ideal_clusters" in item and i < len(item["ideal_clusters"]):
                vocab = item["ideal_clusters"][i]
            
            norm_item = {
                "id": f"{item['id']}_{i}",
                "level": level,
                "category": "Academic Discussion",
                "title": f"{item['topic']} - Case {i+1}",
                "prompt": p_text,
                "type": "discussion",
                "time": "2 min",
                "tips": ["State your position early", "Use connecting words", "Provide academic evidence"],
                "vocab": vocab,
                "model_answer": "Sample answer placeholder.",
                "follow_up": ["What other factors contribute to this issue?", "How might the future look differently?"]
            }
            normalized_data.append(norm_item)
    else:
        # Already normalized or follows the newer schema
        normalized_data.append(item)

# Merge new_items without duplicates
existing_ids = {item['id'] for item in normalized_data}
...
print(f"Normalized and expanded speaking prompts. Total items: {len(normalized_data)}")
